...
export function Stepper({ currentStep, steps, onBack, isFirstStep, children }: StepperProps) {
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="flex items-center">
              ...
              <span className={`ml-1.5 text-sm font-medium md:block hidden ${isActive ? 'text-green-600' : 'text-gray-500'}`}>
                {step.label}
              </span>
              {index < steps.length - 1 && (
                <div className={`mx-2 w-8 h-0.5 ${index < currentIndex ? 'bg-green-600' : 'bg-gray-200'}`} />
              )}
            </div>
          );
        })}
