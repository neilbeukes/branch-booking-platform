...
export const appointments = {
  list: (): Promise<BookingListItem[]> =>
    api<BookingListItem[]>("/appointments"),
  ...
  getByReferenceAndEmail: (
    ref: string,
    email: string,
  ): Promise<BookingListItem> =>
    api<BookingListItem>(
      `/appointments/${encodeURIComponent(ref)}?email=${encodeURIComponent(email)}`,
    ),
  cancel: async (ref: string, email: string): Promise<void> => {
    const res = await fetch(
      `${API_BASE}/appointments/${encodeURIComponent(ref)}`,
      {
        method: "DELETE",
